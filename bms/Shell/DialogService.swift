...
class DialogService {
    ...
    static func initialize(getTopViewController: (() -> UIViewController?)? = nil){
        ...
        SwiftEventBus.onMainThread(self, name: Consts.instance.dialogShowLoading) { result in DialogService.showLoading(result:result?.object) }
        SwiftEventBus.onMainThread(self, name: Consts.instance.dialogHideLoading) { result in DialogService.hideLoading() }
        SwiftEventBus.onMainThread(self, name: Consts.instance.showAlert) { result in DialogService.showAlert(result: result?.object) }
        SwiftEventBus.onMainThread(self, name: Consts.instance.showActionSheet) { result in DialogService.showActionSheet(result: result?.object) }
    }
