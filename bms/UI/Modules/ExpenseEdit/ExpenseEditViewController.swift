...
class ExpenseEditViewController: BaseViewController {
    ...
    override func bind() {
        ...
        viewModel.onSetCurrencies = { [weak self] items in
            guard let self = self else { return }
            self.createCurrencyPickerView(items: items)
        }
        viewModel.onSetPaymentTypes = { [weak self] items in
            guard let self = self else { return }
            self.createPaymentTypePickerView(items: items)
        }
        viewModel.onSetCurrentDate = { [weak self] date in
            guard let self = self else { return }
            self.datePickerTextField.text = date.toString()
        }
        viewModel.onSetCurrentCategory = { [weak self] value in
            guard let self = self else { return }
            self.pickerCategoryTextField.text = value.name
        }
        viewModel.onSetCurrentCurrency = { [weak self] value in
            guard let self = self else { return }
            self.currencyTextField.text = value.name
        }
        viewModel.onSetCurrentPaymentType = { [weak self] value in
            guard let self = self else { return }
            self.paymentTypeTextField.text = value.name
        }
        viewModel.onSetCurrentItem = { [weak self] value in
            guard let self = self else { return }
        ...
        viewModel.didSetPageState = { [weak self] status in
            guard let self = self else { return }
            switch status {
            case .edit:
                self.title = "Editing expenses"
                let button = UIBarButtonItem(image: UIImage(named: "trash"),
                                             style: .plain,
                                             target: self,
                                             action: #selector(self.deleteButtonAction))
                self.navigationItem.rightBarButtonItems = [button]
            case .new:
                self.title = "Adding expenses"
                let button = UIBarButtonItem(image: UIImage(named: "cancel"),
                                             style: .plain,
                                             target: self,
    ...
    func getCategoriesPickerDelegate(items: [ExpenseCategoryObject]) -> PickerSettings {
        let pickerDelegate = PickerSettings(
            ...
            getDisplayValue: { value in
                let item = value as! ExpenseCategoryObject
                return item.name
        },
            didSelectRow: { [weak self] value in
                let item = value as! ExpenseCategoryObject
